...
namespace iRecognizeYou
{
	...
	public partial class Learn : Form
	{
		...
		private void calculateEigenValues( )
		{
			...
			EigenObjectRecognizer eor = new EigenObjectRecognizer(grayFaces,labels, ref cryteria);
		...
		private void saveRememberedFaces( )
		{
			...
			{
				if(grayFaces[i] != null)
				grayFaces [i].Save(createFileName(splitted [0], splitted [1], i));// ?
			}
		}
		/// <summary>
		...
		/// <returns></returns>
		private String getUserLabel( String filename )
		{
